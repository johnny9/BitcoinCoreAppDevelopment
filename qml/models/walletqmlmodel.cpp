...
bool WalletQmlModel::prepareTransactionInternal(const std::optional<QString>& passphrase)
{
    ...
    if (m_custom_fee_enabled) {
        ...
        if (!custom_fee_rate_per_kvb.has_value()) {
            ...
            setTransactionStatus(tr("Enter a valid custom fee rate."));
            return false;
        }
        coin_control.m_confirm_target.reset();
    ...
    const QString error = LocalizedString(util::ErrorString(result));
...
bool WalletQmlModel::sendTransactionInternal(const std::optional<QString>& passphrase)
{
    ...
    if (m_wallet->hasExternalSigner()) {
        interfaces::WalletValueMap value_map;
        interfaces::WalletOrderForm order_form;
        m_wallet->commitTransaction(preview_tx, value_map, order_form);
        clearTransactionStatus();
        return true;
    }
