...
public:
    ...
    QString displayUnitLabel() const { return m_display_unit == 1 ? QStringLiteral("sat") : QStringLiteral("BTC"); }
    ...
    QString languageSummary() const { return m_language.isEmpty() ? QStringLiteral("System default") : m_language; }
    QStringList availableLanguages() const { return {QString{}, QStringLiteral("de"), QStringLiteral("es"), QStringLiteral("fr")}; }
    Q_INVOKABLE QString languageLabel(const QString& tag) const
    {
        if (tag.isEmpty()) return QStringLiteral("System default");
        if (tag == QStringLiteral("de")) return QStringLiteral("Deutsch — German");
        if (tag == QStringLiteral("es")) return QStringLiteral("Español — Spanish");
        if (tag == QStringLiteral("fr")) return QStringLiteral("Français — French");
        return tag;
    }
